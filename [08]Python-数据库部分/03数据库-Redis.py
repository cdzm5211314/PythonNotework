...
### Redis数据库的链接[包含远程链接]: cmd
# 注: 远程链接需要注释掉: 配置文件xxx.conf中的bind 127.0.0.1信息
# 链接后,使用ping命令查看链接是否正常(PONG),然后可以设置寄修改密码: config set requirepass password
## 无密码链接:
# redis-cli
# redis-cli -h 192.168.23.145 -p 6379
## 有密码链接:
# redis-cli ---> auth "password"
# redis-cli -a "password"
# redis-cli -h 192.168.23.145 -p 6379 ---> auth "password"
# redis-cli -h 192.168.23.145 -p 6379 -a "password"



### Redis数据库是key-value的数据结构,值的类型分为5种: 字符串(string),哈希(hash),列表(list),集合(set),有序集合(zset)
