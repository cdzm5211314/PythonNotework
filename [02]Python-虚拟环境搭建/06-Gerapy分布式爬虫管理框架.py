...
# Gerapy 是一款分布式爬虫管理框架,支持Python3,基于 Scrapy、Scrapyd、Scrapyd-Client、Scrapy-Redis、Scrapyd-API、Scrapy-Splash、Jinjia2、Django、Vue.js 开发

# 1.Scrapy: 是一个基于Twisted的异步IO框架,使用该框架就不需要等待当前URL抓取完毕之后在进行下一个URL的抓取,抓取效率可以提高很多
# 2.Scrapy-Redis: 虽然Scrapy框架是异步加多线程的,但是我们只能在一台主机上运行,爬取效率还是有限的,Scrapy-Redis组件为我们提供了Scrapy分布式的队列,调度器,去重等等功能,有了它,我们就可以将多台主机组合起来,共同完成一个爬取任务,抓取的效率又提高了
...
# 4.Gerapy: 是一个基于Scrapyd,Scrapyd API,Django,Vue.js搭建的分布式爬虫管理框架;简单说就是用上述的Scrapyd工具是在命令行进行操作,而Gerapy将命令行和图形界面进行了对接,我们只需要点击按钮就可完成部署,启动,停止,删除的操作。
