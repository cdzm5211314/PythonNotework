...
### 经典的设计模式共归纳了23种:分为三大类(创建型,结构型,行为型)
    ## 创建型模式(5):提供实例化的方法，为适合的状况提供相应的对象创建方法。
        # 简单工厂模式(Simple Factory) : 不是Gof总结出来的23种模式之一
    ...
    ## 结构型模式(7):通常用来处理实体之间的关系，使得这些实体能够更好地协同工作。
        # 外观模式(Facade)
        # 适配器模式(Adapter)
        # 代理模式(Proxy)
        # 装饰模式(Decorator)
    ...
    ## 行为型模式(11):用于在不同的实体建进行通信，为实体之间的通信提供更容易，更灵活的通信方法。
...
# 24种设计模式：
    ...
    # 二、结构型模式
        ...
        # 3、组合模式(composite pattern): 允许你将对象组合成树形结构来表现”整体/[07]Python-设计模式部分”层次结构. 组合能让客户以一致的方式处理个别对象以及对象组合.
