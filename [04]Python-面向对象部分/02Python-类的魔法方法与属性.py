...
### 魔法方法(内置对象方法):
## 实例化对象方法: __new__(cls, *args, **kwargs)
# 作用: 实例化一个对象时最先调用__new__()方法,并返回一个实例,然后调用初始化方法__init__()

## 初始化方法: __init__(self [,形参列表])
# 作用: 对新创建的对象添加实例变量(属性)或相应的资源

## 对象字符串方法: __str__(self)
# 作用: 必须返回一个字符串,打印对象的描述信息

## 析构方法: __del__(self)
# 作用: 析构方法在对象销毁时被自动调用,清理此对象所占用的资源

## 魔法方法: __call__(self)
# 作用: 对于call方法的执行是由对象后加括号触发的;如 对象() 或 类()()
