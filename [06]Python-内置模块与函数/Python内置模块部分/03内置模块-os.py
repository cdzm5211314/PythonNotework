...
### os : 提供很多函数与操作系统进行调用交互
# 注: 确保使用import os而不是from os import *
# 这样可以防止函数os.open()覆盖内建函数open()，两者之间的操作是很不同的。
...
### os模块详解
# os.rename(src, dst)   重命名(文件与文件夹)
# os.remove(file)       删除文件
# os.mkdir(dirname)     创建文件夹
# os.rmdir(dirname)     删除文件夹
# os.getcwd()           返回当前文件所在工作目录,即当前Python脚本工作的目录路径
# os.chdir(dirname)     改变目录路径,如在AA目录里面创建BB目录(切换到AA目录,创建BB目录)
# os.listdir()          返回当前目录下的所有文件和目录名的列表
# os.listdir(dirname)   返回指定目录下的所有文件和目录名的列表
